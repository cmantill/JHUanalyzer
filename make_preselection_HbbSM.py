...
if __name__ == "__main__":
    ...
    #############################
    if jobs!=1:
        f = ROOT.TFile( "HbbSM"+options.year+"_"+options.set+"_job"+options.job+"of"+options.njobs+mod+'_'+doubleB_short+'_'+options.region+".root", "recreate" )
    else:
        f = ROOT.TFile( "HbbSM"+options.year+"_"+options.set+mod+'_'+doubleB_short+'_'+options.region+".root", "recreate" )
    f.cd()
    ...
    if not isData:
        ...
        hname = 'data_obs'
    Hbb_cutflow = ROOT.TH1D('%s_cutflow'%hname, 'Hbb_cutflow', 10, 0.5, 10.5)
    ...
    N2_map = ROOT.TH3F('%s_n2ddt_map'%hname,'%s_n2ddt_map'%hname,
                  ...
                  200, 0, 0.5)
    N2_map.Sumw2()
    Hbb_doubleB = ROOT.TH1F('%s_doubleB'%hname,''+doubleB_title+' tag',40,0,1)
    Hbb_doubleB.Sumw2()
    Hbb_deepAK8 = ROOT.TH1F('%s_deepAK8'%hname,'deepAK8', 40,0,1)
    Hbb_deepAK8.Sumw2()
    Hbb_pT = ROOT.TH1F('%s_pT'%hname,'hbb pT',40,200,1000)
    Hbb_pT.Sumw2()
    Hbb_pT_450 = ROOT.TH1F('%s_pT_450'%hname,'hbb pT 450',40,400,1000)
    Hbb_pT_450.Sumw2()
    Hbb_mSD = ROOT.TH1F('%s_mSD'%hname,'hbb mSD',20,40,220)
    Hbb_mSD.Sumw2()
    Hbb_mSD_450 = ROOT.TH1F('%s_mSD_450'%hname,'hbb mSD 450',20,40,220)
    Hbb_mSD_450.Sumw2()
    Hbb_mSD_uncorr =  ROOT.TH1F('%s_mSD_uncorr'%hname,'hbb mSD uncorr',20,40,220)
    Hbb_mSD.Sumw2()
    Hbb_mSD_passM_n2ddt = ROOT.TH1F('%s_mSD_passM_n2ddt'%hname,'hbb mSD',20,40,220)
    Hbb_mSD.Sumw2()
    Hbb_mSD_failM_n2ddt = ROOT.TH1F('%s_mSD_failM_n2ddt'%hname,'hbb mSD',20,40,220)
    Hbb_mSD.Sumw2()
    Hbb_eta = ROOT.TH1F('%s_eta'%hname,'hbb eta',40,-3,3)
    Hbb_eta.Sumw2()
    Hbb_rho = ROOT.TH1F('%s_rho'%hname,'hbb rho',40,-7,-1)
    Hbb_rho.Sumw2()
    Hbb_n2ddt_new = ROOT.TH1F('%s_n2ddt_new'%hname,'hbb n2ddt new',40,-0.5,0.5)
    Hbb_n2ddt_new.Sumw2()
    Hbb_n2 = ROOT.TH1F('%s_n2'%hname,'hbb n2',40,0.,0.8)
    Hbb_n2.Sumw2()
    Muon_pT = ROOT.TH1F('%s_muonpT'%hname,'muon pT',40,0,500)
    Muon_pT.Sumw2()
    Muon_eta = ROOT.TH1F('%s_muoneta'%hname,'muon eta',40,-2.5,2.5)
    Muon_eta.Sumw2()
    Muon_phi = ROOT.TH1F('%s_muonphi'%hname,'muon phi',40,-4,4)
    Muon_phi.Sumw2()
    Muon_dphi = ROOT.TH1F('%s_muondphi'%hname,'muon dphi',40,-4,4)
    Muon_dphi.Sumw2()
    Muon_pfRelIso04_all = ROOT.TH1F('%s_muonpfRelIso04'%hname,'muon reliso 0.4',40,0,1)
    ...
    for wp in WPs:
        ...
        for cat in passfail:
            ...
            hists[wp][cat] = ROOT.TH1F(histname, histtitle, len(Mass_binBoundaries)-1, array('d', Mass_binBoundaries))
    ...
    if runOthers == True:
        if not isData:
            ...
            for wp in WPs:
                for sys in systs:
                    for cat in passfail:
                        hists[wp]['%s_%s'%(cat,sys)] = ROOT.TH1F("%s_%s%s_%s"%(hname,wp,cat,sys), "mass of Hbb vs pT - %s %s %s"%(cat,sys,wp), len(Mass_binBoundaries)-1,  array('d', Mass_binBoundaries))
    ...
    for entry in range(lowBinEdge,highBinEdge):        
        ...
        if len(muonColl) > 0:
            ...
            if( muonColl[0].looseId and muon_pfRelIso04_all < 0.40):
                ...
                if(muon_pt > 55) and (abs(muon_eta) < 2.1):
                    Hbbsel['muonkin'] = True
                    if(muon_dphi > 2.0*ROOT.TMath.Pi()/3.0):
                       Hbbsel['muon'] = True
        ...
        if len(ak4JetsColl)>0:
            for i in range(0,len(ak4JetsColl)):
                if ak4JetsColl[i].btagDeepB > 0.6324 and ak4JetsColl[i].pt > 50 and abs(ak4JetsColl[i].eta) < 2.5:
                    ak4vec =  ROOT.TLorentzVector(); 
                    ak4vec.SetPtEtaPhiM(ak4JetsColl[i].pt,ak4JetsColl[i].eta,ak4JetsColl[i].phi,ak4JetsColl[i].mass)
                    #ak4vec.SetPt(ak4JetsColl[i].pt)
                    #ak4vec.SetEta(ak4JetsColl[i].eta)
                    #ak4vec.SetPhi(ak4JetsColl[i].phi)
                    #ak4vec.SetM(ak4JetsColl[i].mass)
                    if(ak4vec.DeltaR(jet0) > 0.8):
                        Hbbsel['btag'] = True

        # rho
        if ak8JetsColl[0].pt >= 200:
            Hbbrho = 2*math.log(jet0.M()/jet0.Pt())
        ...
        if not isData:
            ...
            if muon_pt>0 and abs(muon_eta)>0:
                ...
                muTrigs = Presel_Functions.Hist2d_Lookup(muPtForTrig,muEtaForTrig,mutrig_eff)
                ...
                muIds = Presel_Functions.Hist2d_Lookup(muPtForId,muEtaForId,muid_eff)
                weights['muid']['nom'] = muIds[0]
                weights['muid']['up'] = muIds[1]
                weights['muid']['down'] = muIds[2]

                muIsos = Presel_Functions.Hist2d_Lookup(muPtForId,muEtaForId,muiso_eff)
        ...
        #########################################
        nominalweight = Presel_Functions.Weightify(weights,'nominal',['Pu','mutrigger','muid','muiso'])
        if not isData:
            if ak8JetsColl[0].msoftdrop > 20 and ak8JetsColl[0].pt > 200 and not Hbbsel['leptonExists']:
                N2_map.Fill(2*math.log(ak8JetsColl[0].msoftdrop/ak8JetsColl[0].pt),ak8JetsColl[0].pt,ak8JetsColl[0].n2b1,norm_weight*nominalweight)
        if preselection:
                ...
                for wp in WPs:
                    for cat in ['pass','fail']:
                        ...
                        for sys in ['Pu','mutrigger','muiso','muid']:
                            if Hbbsel['%s_%s'%(cat,wp)]:
                                hists[wp]['%s_%sUp'%(cat,sys)].Fill(jet0.M(),norm_weight*Presel_Functions.Weightify(weights,'%s_up'%sys,['Pu','mutrigger','muid','muiso']))
                                hists[wp]['%s_%sDown'%(cat,sys)].Fill(jet0.M(),norm_weight*Presel_Functions.Weightify(weights,'%s_down'%sys,['Pu','mutrigger','muid','muiso']))
